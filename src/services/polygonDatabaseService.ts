...
export class PolygonDatabaseService {
  ...
  private async initializeOptionsTable(): Promise<void> {
    const tableId = 'polygon_options';
    
    // Schema definition for polygon_options table
    // NEW Primary Key: (date, contract_id)
    // - date: Trading date in EST (one row per contract per trading day)
    ...
    const schema = [
      ...
      // Timestamps - UPDATED
      { name: 'insert_timestamp', type: 'TIMESTAMP', mode: 'NULLABLE' }, // When data was originally fetched
      { name: 'last_updated', type: 'TIMESTAMP', mode: 'REQUIRED' }, // When row was last modified
      ...
      // Day Field Child Attributes - EXPANDED
      ...
      { name: 'previous_close', type: 'FLOAT64', mode: 'NULLABLE' },
      { name: 'vwap', type: 'FLOAT64', mode: 'NULLABLE' },
      { name: 'day_last_updated', type: 'TIMESTAMP', mode: 'NULLABLE' },
      { name: 'day_volume', type: 'INT64', mode: 'NULLABLE' },
      ...
      // Last Quote Field Child Attributes - NEW
      { name: 'quote_bid', type: 'FLOAT64', mode: 'NULLABLE' },
      { name: 'quote_ask', type: 'FLOAT64', mode: 'NULLABLE' },
      { name: 'quote_bid_size', type: 'INT64', mode: 'NULLABLE' },
      { name: 'quote_ask_size', type: 'INT64', mode: 'NULLABLE' },
      { name: 'quote_last_updated', type: 'TIMESTAMP', mode: 'NULLABLE' },
      { name: 'quote_last_exchange', type: 'STRING', mode: 'NULLABLE' },
      { name: 'quote_midpoint', type: 'FLOAT64', mode: 'NULLABLE' },
      { name: 'quote_timeframe', type: 'STRING', mode: 'NULLABLE' },
      { name: 'quote_bid_exchange', type: 'STRING', mode: 'NULLABLE' },
      
      // Last Trade Field Child Attributes - NEW
      { name: 'last_trade_price', type: 'FLOAT64', mode: 'NULLABLE' },
      { name: 'last_trade_size', type: 'INT64', mode: 'NULLABLE' },
      { name: 'last_trade_exchange', type: 'STRING', mode: 'NULLABLE' },
      { name: 'last_trade_conditions', type: 'STRING', mode: 'NULLABLE' },
      { name: 'last_trade_timestamp', type: 'TIMESTAMP', mode: 'NULLABLE' },
      
      // Underlying Asset Field Child Attributes - NEW
      { name: 'underlying_ticker', type: 'STRING', mode: 'NULLABLE' },
  ...
  private async initializeStockTable(): Promise<void> {
  ...
  private async initializeCryptoTable(): Promise<void> {
  ...
  private async initializeRealTimeTable(): Promise<void> {
  ...
  private async createTableIfNotExists(tableId: string, schema: any[]): Promise<void> {
    try {
  ...
  private async validateCompositePrimaryKey(tableId: string, contractId: string, date: Date): Promise<boolean> {
    try {
      if (tableId === 'polygon_options') {
        const dataset = this.bigquery.dataset(this.datasetId);
        const table = dataset.table(tableId);
        
        // Check if a record with the same date and contract_id already exists
        const query = `
  ...
  private calculateOptionsScore(contract: any): number | null {
    try {
  ...
  private formatStockRow(data: StockSnapshot): any {
    return {
      timestamp: new Date(),
      ticker: data.ticker,
  ...
  private formatCryptoRow(data: CryptoSnapshot): any {
    return {
      timestamp: new Date(),
      ticker: data.ticker,
  ...
  private formatRealTimeRow(data: RealTimeOptionsData): any {
    return {
      timestamp: new Date(),
  ...
  async storeOptionsData(data: OptionContractSnapshot | OptionChainSnapshot | UnifiedOptionsSnapshot): Promise<void> {
    try {
    ...
    try {
      ...
      rows.forEach((row, index) => {
        params.push(
          ...
          { name: `strike_price_${index}`, value: row.strike_price },
          { name: `expiration_date_${index}`, value: row.expiration_date },
          { name: `exercise_style_${index}`, value: row.exercise_style },
          { name: `shares_per_contract_${index}`, value: row.shares_per_contract },
          { name: `primary_exchange_${index}`, value: row.primary_exchange },
          { name: `currency_${index}`, value: row.currency },
          { name: `underlying_price_${index}`, value: row.underlying_price },
          { name: `underlying_timestamp_${index}`, value: row.underlying_timestamp },
          { name: `delta_${index}`, value: row.delta },
          { name: `gamma_${index}`, value: row.gamma },
          { name: `theta_${index}`, value: row.theta },
          { name: `vega_${index}`, value: row.vega },
          { name: `rho_${index}`, value: row.rho },
          { name: `lambda_${index}`, value: row.lambda },
          { name: `epsilon_${index}`, value: row.epsilon },
          { name: `charm_${index}`, value: row.charm },
          { name: `vanna_${index}`, value: row.vanna },
          { name: `volga_${index}`, value: row.volga },
          { name: `bid_${index}`, value: row.bid },
          { name: `ask_${index}`, value: row.ask },
          { name: `bid_size_${index}`, value: row.bid_size },
          { name: `ask_size_${index}`, value: row.ask_size },
          { name: `mid_price_${index}`, value: row.mid_price },
          { name: `spread_${index}`, value: row.spread },
          { name: `spread_percentage_${index}`, value: row.spread_percentage },
          { name: `last_price_${index}`, value: row.last_price },
          { name: `last_size_${index}`, value: row.last_size },
          { name: `last_trade_exchange_${index}`, value: row.last_trade_exchange },
          { name: `last_trade_conditions_${index}`, value: row.last_trade_conditions },
          { name: `volume_${index}`, value: row.volume },
          { name: `open_interest_${index}`, value: row.open_interest },
          { name: `implied_volatility_${index}`, value: row.implied_volatility },
          { name: `historical_volatility_${index}`, value: row.historical_volatility },
          { name: `min_av_${index}`, value: row.min_av },
          { name: `min_av_timestamp_${index}`, value: row.min_av_timestamp },
          { name: `prev_day_volume_${index}`, value: row.prev_day_volume },
          { name: `prev_day_open_interest_${index}`, value: row.prev_day_open_interest },
          { name: `prev_day_high_${index}`, value: row.prev_day_high },
          { name: `prev_day_low_${index}`, value: row.prev_day_low },
          { name: `prev_day_close_${index}`, value: row.prev_day_close },
          { name: `prev_day_vwap_${index}`, value: row.prev_day_vwap },
          { name: `days_to_expiration_${index}`, value: row.days_to_expiration },
          { name: `time_value_${index}`, value: row.time_value },
          { name: `intrinsic_value_${index}`, value: row.intrinsic_value },
          { name: `extrinsic_value_${index}`, value: row.extrinsic_value },
          { name: `moneyness_${index}`, value: row.moneyness },
          { name: `leverage_${index}`, value: row.leverage },
          { name: `probability_itm_${index}`, value: row.probability_itm },
          { name: `probability_otm_${index}`, value: row.probability_otm },
          { name: `max_loss_${index}`, value: row.max_loss },
          { name: `max_profit_${index}`, value: row.max_profit },
          { name: `break_even_price_${index}`, value: row.break_even_price },
          { name: `score_${index}`, value: row.score },
          { name: `quote_timestamp_${index}`, value: row.quote_timestamp },
          { name: `trade_timestamp_${index}`, value: row.trade_timestamp },
          { name: `participant_timestamp_${index}`, value: row.participant_timestamp },
          { name: `chain_timestamp_${index}`, value: row.chain_timestamp },
          { name: `exchange_${index}`, value: row.exchange },
          { name: `conditions_${index}`, value: row.conditions },
          { name: `market_center_${index}`, value: row.market_center },
          { name: `tick_size_${index}`, value: row.tick_size },
          { name: `lot_size_${index}`, value: row.lot_size },
          { name: `is_penny_${index}`, value: row.is_penny },
          { name: `is_weekly_${index}`, value: row.is_weekly },
          { name: `is_monthly_${index}`, value: row.is_monthly },
          { name: `is_quarterly_${index}`, value: row.is_quarterly },
  ...
  private async upsertStockRow(row: any): Promise<void> {
    try {
      const query = `
  ...
  private async upsertCryptoRow(row: any): Promise<void> {
    try {
      const query = `
  ...
  private async upsertRealTimeRow(row: any): Promise<void> {
    try {
      const query = `
  ...
  async storeStockData(data: StockSnapshot): Promise<void> {
    try {
  ...
  async storeCryptoData(data: CryptoSnapshot): Promise<void> {
    try {
  ...
  async storeRealTimeData(data: RealTimeOptionsData): Promise<void> {
    try {
  ...
  async storeDataCollection(collection: PolygonDataCollection): Promise<void> {
    try {
      const { data, source } = collection;
      
      if (source === 'polygon_options') {
        await this.storeOptionsData(data.response as any);
      } else if (source === 'polygon_stock') {
        await this.storeStockData(data.response as StockSnapshot);
      } else if (source === 'polygon_crypto') {
        await this.storeCryptoData(data.response as CryptoSnapshot);
      }
...
export const polygonDatabaseService = new PolygonDatabaseService(); 