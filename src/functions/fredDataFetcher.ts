...
// Comprehensive FRED metrics - exploring all major economic indicators
const FRED_METRICS = [
  // Interest Rates
  {
...
export const fredDataFetcher = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    for (const metric of FRED_METRICS) {
      try {
        ...
        const data = await apiClient.fetchFREDData(metric.endpoint, {
          ...metric.parameters,
          limit: 100,
          sort_order: 'desc'
        });
        ...
        try {
          await databaseService.storeFREDData(data, metric.name, {
